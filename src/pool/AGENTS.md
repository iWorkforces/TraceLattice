...
Optional HTTP-layer session isolation. Each slot is a `SessionServer` from `serverFactory`. **Not** `HistoryManager`'s `SessionManager` (thought-history keyed by `session_id`).
