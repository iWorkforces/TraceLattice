...
| 1 | Event-stream endpoint custom path | Removed. Streamable HTTP `GET` on the MCP path returns 405. |
...
| Pooled HTTP identity and routing | Dedicated event-stream pooling is removed. Streamable HTTP is POST-only. | 15 | 16, 17 |
...
| D3 | APPROVED | Permit one active writer per canonical file-storage directory across instances and processes. Acquire ownership before mutable load, serialize every mutation, and release only ownership held by that instance. Competing writers fail explicitly. Never auto-steal a stale-looking lock. | 6, 15; supported-topology gate, not distributed coordination. |
...
| D8 | WITHDRAWN | Event-stream HTTP transport and pooled message-path routing are removed. Streamable HTTP accepts POST only; `GET` on the MCP path is 405. | 15 |
...
| Writer topology | One canonical directory has one active writer. | Competing instances/processes must fail before mutation or conflicting acquisition. |
...
| Streamable HTTP methods | MCP path is POST-only. | `GET`/`PUT`/`PATCH` on the MCP path must return 405. |
